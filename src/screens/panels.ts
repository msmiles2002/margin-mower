...
import { MAX_STARS, type PropertyScore } from '../rules/scoring';
...
import { button, el, logo, showPanel, waitFor } from './dom';
...
export function showPropertyCard(
  ...
  return waitFor<void>(overlay, (done) => [
    el('h1', { text: property.name }),
    outcome(copy),
    el('div', { className: 'stars', text: starString(score.stars, MAX_STARS) }),
    ...headlineBlock(copy),
